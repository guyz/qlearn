...
class Learner:
    ...
    def __init__(self):
        ...
        self.Q = np.zeros((2,SCREEN_WIDTH/BINSIZE+1,SCREEN_HEIGHT/BINSIZE+1,SCREEN_HEIGHT/BINSIZE2+1,VSTATES))
        self.k = np.zeros((2,SCREEN_WIDTH/BINSIZE+1,SCREEN_HEIGHT/BINSIZE+1,SCREEN_HEIGHT/BINSIZE2+1,VSTATES)) # number of times action a has been taken from state s
        self.iters = 0
        self.mem = [0, 0]
        self.scores = []
        self.best_score = 50
    ...
    def action_callback(self, state):
        ...
        M = state['monkey']['top'] / BINSIZE2
        V = state['monkey']['vel'] / 20
        # V = (state['monkey']['top']-self.mem[0]) / 20
        if np.abs(V) > 2:
            V = np.sign(V)*2
        ...
        if not self.last_action == None:
            ...
            m = self.last_state['monkey']['top'] / BINSIZE2
            v = self.last_state['monkey']['vel'] / 20
            # v = (self.last_state['monkey']['top']-self.mem[0]) / 20
            if np.abs(v) > 2:
                v = np.sign(v)*2

            max_Q = np.max(self.Q[:,D,T,M,V])
            new_action = 1 if self.Q[1][D,T,M,V] > self.Q[0][D,T,M,V] else 0
            
            # epsilon-greedy
            if self.k[new_action][D,T,M,V] > 0:
                eps = EPS_FACTOR/self.k[new_action][D,T,M,V]
            else:
                eps = EPS_FACTOR
            ...
            ALPHA = 1/self.k[self.last_action][d,t,m,v]
            self.Q[self.last_action][d,t,m,v] += ALPHA*(self.last_reward+GAMMA*max_Q-self.Q[self.last_action][d,t,m,v])
            
        self.mem[0] = state['monkey']['top']

        self.last_action = new_action
        self.last_state  = state
        self.k[new_action][D,T,M,V] += 1
        return new_action
